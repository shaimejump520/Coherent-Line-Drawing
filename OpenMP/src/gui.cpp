...
bool MyApp::OnInit() {
	MyFrame *frame = new MyFrame("Coherent Line Drawing | PP-f18 Group 4", wxPoint(50, 50), wxSize(1000, 730));
	//frame->Maximize(true);
	frame->Show(true);
...
MyFrame::MyFrame(const wxString& title, const wxPoint& pos, const wxSize& size)
	...
	demo = new wxButton(toolbar1, BUTTON_Demo, _T("Demo"), wxDefaultPosition, wxSize(100, 30), 0);
...
void MyFrame::OnDemo(wxCommandEvent& event) {
	...
	time_t beginTime;// for logging current time
	time_t bbeginTime;// for logging current time
	struct tm *localBgTime;// for logging current time
	time(&bbeginTime);                   // Get the current time
	time(&beginTime);                   // Get the current time
	// localBgTime = localtime(&beginTime);  // Convert the current time to the local time

	// wxString tt;
	// tt.Printf("*******Begin time: %02d : %02d : %02d*******", (localBgTime->tm_hour),(localBgTime->tm_min),(localBgTime->tm_sec));
	// addlog(tt, wxColour(*wxRED));
	time_t endTime;// for logging current time
	time(&endTime);

	addlog("*******Demo start*******", wxColour(*wxRED));
	wxString duration;
	//line parameters setting
	slider_rho->SetValue(9900);
	rho_handler();
	slider_sigma1->SetValue(3299);
	sig_m_handler();
	slider_sigma2->SetValue(997);
	sig_c_handler();
	slider_t->SetValue(8810);
	tau_handler();
	...
	refine_ETF_handler();
	time(&endTime);
	duration.Printf("*******smooth_ETF spend time: %.10f seconds*******", difftime(endTime, beginTime));
	addlog(duration, wxColour(*wxRED));
	refine_ETF_handler();
	refine_ETF_handler();

	//change mode to CLD
	processingBox->SetValue(MODE_CLD);
	change_mode_handler();

	//2 times iterative fDoG
	time(&beginTime);
	iterative_fDoG_handler();
	time(&endTime);
	duration.Printf("*******fDoG spend time: %.10f seconds*******", difftime(endTime, beginTime));
	addlog(duration, wxColour(*wxRED));
	iterative_fDoG_handler();
	addlog("*******Demo done*******", wxColour(*wxRED));

	       
	// struct tm *localEdTime;// for logging current time            // Get the current time
	// localEdTime = localtime(&endTime);  // Convert the current time to the local time

	// wxString rrrr;
	// rrrr.Printf("*******End time: %02d : %02d : %02d*******", (localEdTime->tm_hour),(localEdTime->tm_min),(localEdTime->tm_sec));
	// addlog(rrrr, wxColour(*wxRED));



	time(&endTime);
	duration.Printf("*******total spend time: %.10f seconds*******", difftime(endTime, bbeginTime));
	addlog(duration, wxColour(*wxRED));
...
void MyFrame::refine_ETF_handler(){
...
void MyFrame::iterative_fDoG_handler(){
...
void MyFrame::change_mode_handler(){
